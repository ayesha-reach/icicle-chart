...
st.markdown("""
    <style>
        ...
        /* Target the specific white space issue */
...
st.markdown('<div class="main-title">📊 Customer Chain Analysis - Downstream & Upstream Beta Version</div>', unsafe_allow_html=True)
st.markdown("<div style='height: 0px; margin: 0; padding: 0;'></div>", unsafe_allow_html=True)

# ---------------------- Duration and Hop Filter Section ----------------------
st.markdown('<div class="filter-section">', unsafe_allow_html=True)

filter_col1, filter_col2 = st.columns(2)

with filter_col1:
    duration = st.selectbox("Select Duration", ["1 Month", "3 Months", "6 Months", "1 Year"])
...
# Dynamic hop filter based on actual data
st.markdown('<div class="filter-section">', unsafe_allow_html=True)
...
with col1:
    if upstream_available:
        ...
        fig_upstream = px.icicle(
            ...
            values=values_up,
            ids=ids_up,
            title=f"📈 Upstream Chain – {display_name} – {duration}{title_suffix}",
        ...
        st.plotly_chart(fig_upstream, use_container_width=True)
    else:
        st.info("No upstream data available for this customer")
...
with col2:
    if downstream_available:
        ...
        fig_downstream = px.icicle(
            ...
            values=values_down,
            ids=ids_down,
            title=f"📊 Downstream Chain – {display_name} – {duration}",
        ...
        st.plotly_chart(fig_downstream, use_container_width=True)
    else:
        st.info("No downstream data available for this customer")
