...
-- ,fleet, fleet_id, forwarded_1, forwarded_1_id, forwarded_2, forwarded_2_id, forwarded_3, forwarded_3_id, forwarded_4, forwarded_4_id, forwarded_5, forwarded_5_id, forwarded_6, forwarded_6_id
-- from union_table_
...
, base_chain AS (
  ...
  CASE WHEN forwarded_6 IS NOT NULL THEN CONCAT(CAST(forwarded_6 AS TEXT), concat('|', CAST(forwarded_6_id AS TEXT)) ) else null END as col_1 , 
  CASE WHEN forwarded_5 IS NOT NULL THEN CONCAT(CAST(forwarded_5 AS TEXT), concat('|', CAST(forwarded_5_id AS TEXT)) ) else null END as col_2 , 
  CASE WHEN forwarded_4 IS NOT NULL THEN CONCAT(CAST(forwarded_4 AS TEXT), concat('|', CAST(forwarded_4_id AS TEXT)) ) else null END as col_3 , 
  CASE WHEN forwarded_3 IS NOT NULL THEN CONCAT(CAST(forwarded_3 AS TEXT), concat('|', CAST(forwarded_3_id AS TEXT)) ) else null END as col_4 , 
  CASE WHEN forwarded_2 IS NOT NULL THEN CONCAT(CAST(forwarded_2 AS TEXT), concat('|', CAST(forwarded_2_id AS TEXT)) ) else null END as col_5 , 
  CASE WHEN forwarded_1 IS NOT NULL THEN CONCAT(CAST(forwarded_1 AS TEXT), concat('|', CAST(forwarded_1_id AS TEXT)) ) else null END as col_6 , 
  CASE WHEN fleet IS NOT NULL THEN CONCAT(CAST(fleet AS TEXT), concat('|', CAST(fleet_id AS TEXT))) else null END as col_7 
...
, base_chain_2 AS (
SELECT event_id,
 TRIM(BOTH '~' FROM COALESCE(concat(col_1,'~'), '~' ) || COALESCE(concat(col_2,'~'), '~' ) || COALESCE(concat(col_3,'~'), '~' ) || COALESCE(concat(col_4,'~'), '~' ) 
|| COALESCE(concat(col_5,'~'), '~' ) || COALESCE(concat(col_6,'~'), '~' ) || COALESCE(concat(col_7,'~'), '~' ) ) as reversed_chain
-- *
 from base_chain
),
...
exploded_chain AS (
  SELECT
    b.event_id,
    n.n AS step,
    SPLIT_PART(SPLIT_PART(b.reversed_chain, '~', n.n), '|', 1) AS name,
    SPLIT_PART(SPLIT_PART(b.reversed_chain, '~', n.n), '|', 2) AS id
  FROM base_chain_2 b
  JOIN numbers n ON n.n <= REGEXP_COUNT(b.reversed_chain, '~') + 1
),
...
pivoted AS (
  SELECT
    ...
    MAX(CASE WHEN step = 1 THEN name END) AS forwarded,
    MAX(CASE WHEN step = 1 THEN id END) AS forwarded_from_id,

    MAX(CASE WHEN step = 2 THEN name END) AS forwarded_from_1,
    MAX(CASE WHEN step = 2 THEN id END) AS forwarded_from_1_id,

    MAX(CASE WHEN step = 3 THEN name END) AS forwarded_from_2,
    MAX(CASE WHEN step = 3 THEN id END) AS forwarded_from_2_id,

    MAX(CASE WHEN step = 4 THEN name END) AS forwarded_from_3,
    MAX(CASE WHEN step = 4 THEN id END) AS forwarded_from_3_id,

    MAX(CASE WHEN step = 5 THEN name END) AS forwarded_from_4,
    MAX(CASE WHEN step = 5 THEN id END) AS forwarded_from_4_id,

    MAX(CASE WHEN step = 6 THEN name END) AS forwarded_from_5,
    MAX(CASE WHEN step = 6 THEN id END) AS forwarded_from_5_id,

    MAX(CASE WHEN step = 7 THEN name END) AS forwarded_from_6,
    MAX(CASE WHEN step = 7 THEN id END) AS forwarded_from_6_id
...
,forwarded as customer, forwarded_from_id as customer_id, forwarded_from_1 as customer_1, forwarded_from_1_id as customer_1_id, forwarded_from_2 as customer_2, forwarded_from_2_id as customer_2_id, forwarded_from_3 as customer_3, forwarded_from_3_id as customer_3_id, forwarded_from_4 as customer_4, forwarded_from_4_id as customer_4_id, forwarded_from_5 as customer_5, forwarded_from_5_id as customer_5_id, forwarded_from_6 as customer_6, forwarded_from_6_id as customer_6_id
from pivoted
where forwarded is not null 
group by forwarded, forwarded_from_id, forwarded_from_1, forwarded_from_1_id, forwarded_from_2, forwarded_from_2_id, forwarded_from_3, forwarded_from_3_id, forwarded_from_4, forwarded_from_4_id, forwarded_from_5, forwarded_from_5_id, forwarded_from_6, forwarded_from_6_id
)
, union_all as (
